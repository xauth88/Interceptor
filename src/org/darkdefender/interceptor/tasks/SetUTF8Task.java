...
public abstract class SetUTF8Task extends GenericJSONAsyncTask {

	Context ctx;
	String path = "/test.php";
	
	public SetUTF8Task(Context ctx) {
		super(ctx);
	}
	...
	protected Boolean sendRequest() {
		...
		try {
			...
			response = Net.makeRealHTTPPostRequest(ctx, getEndPointPath()+path, send);
